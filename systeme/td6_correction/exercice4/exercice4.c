...
void* fct_thread_colleur_affiche(void* arg) {
	...
	while(1) {
		...
		printf("Colleur d'affiches n°%d\t : \"Je vais changer les affiches du panneau....\"\n", mon_identifiant);
		//Reservation des places
		for(position = 0; position < (panneaux + mon_identifiant)->nb_places; position++) {
			printf("Colleur d'affiches n°%d\t : Reservation de la place n°%d du panneau....\n", mon_identifiant, position);
			sem_wait(&semaphore_panneau[mon_identifiant]);
			printf("Colleur d'affiches n°%d\t : Place n°%d du panneau reservee....\n", mon_identifiant, position);
		}

		printf("Colleur d'affiches n°%d\t : Changement des affiches du panneau en cours....\n", mon_identifiant);
		sleep(alea_entre_bornes(TEMPS_CHANGEMENT_AFFICHES_MIN, TEMPS_CHANGEMENT_AFFICHES_MAX));
		initialise_panneau(&panneaux[mon_identifiant]);
		printf("Colleur d'affiches n°%d\t : Changement des affiches du panneau terminé !\n", mon_identifiant);

		//Liberation des places
		for(position = 0; position < (panneaux + mon_identifiant)->nb_places; position++) {
			printf("Colleur d'affiches n°%d\t : Liberation de la place n°%d du panneau....\n", mon_identifiant, position);
			sem_post(&semaphore_panneau[mon_identifiant]);
			printf("Colleur d'affiches n°%d\t : Place n°%d du panneau liberee....\n", mon_identifiant, position);
		}
	}
}
