...
void algo(int *solution, int nb_executions, int rang)
{
	...
	int groupe_max;
	etudiant *etudiants_temp = malloc(nb_etudiants * sizeof(etudiant));
	...
	while (repeat < nb_executions)
	{
		...
		{
			printf("Bloque\n");
			printf("Valeur de l evaluation : %d\n",evaluer_etudiants(etudiants));
			if( evaluer_etudiants(etudiants) <= resultat)
			{
				for(i = 0; i < nb_etudiants; i++)
					etudiants_temp[i] = etudiants[i];
			}
			for(i = 0; i < nb_etudiants; i++)
				etudiants[i].groupe = rand()% nb_groupes;
			permutation_bool = 1;
			/*
	...
	for (i = 0; i < nb_etudiants; i++)
		solution[etudiants[i].id] = etudiants_temp[i].groupe;
